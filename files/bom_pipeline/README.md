...
list; anything not covered gets an ISO 9001 check (usually comes back
"Unknown" — there's no unified public registry, this is flagged not
faked) and a contact/channel record (`EMAIL` or `PORTAL`) captured for
Stage 4.
...
line — unsourced parts get a row with a reason, not a blank. This is the
file a client engineer downloads.
